...
type FlashTradeRequest struct {
	...
	DisableCache   bool    `json:"disable_cache"`   // 禁用价格缓存
	UseWebSocket   bool    `json:"use_websocket"`   // 使用WebSocket实时价格
...
func main() {
	fmt.Println("========================================")
	fmt.Println("🚀 批量下发Flash Trade任务脚本 v2.0 (Mac版)")
	fmt.Println("========================================")
	fmt.Println("📡 支持多链 (BSC + Solana) 和多价格模式")
	fmt.Println("⚡ 支持速度模式 (fast/normal/slow)")
	fmt.Println()
...
func getTradeInput() (*FlashTradeRequest, error) {
	...
	// 获取ALPHA ID
	fmt.Print("请输入ALPHA ID (只需输入数字部分，如372): ")
	alphaID, _ := reader.ReadString('\n')
	alphaID = strings.TrimSpace(alphaID)
	if alphaID == "" {
		// 如果用户没有输入ALPHA ID，设置默认值
		fmt.Println("⚠️ 未输入ALPHA ID，使用默认值372")
		alphaID = "372"
	}
	
	// 自动添加ALPHA_前缀
	req.BaseAsset = fmt.Sprintf("ALPHA_%s", alphaID)
	fmt.Printf("✅ 基础资产设置为: %s\n", req.BaseAsset)
	...
	if targetVolumeStr == "" {
		req.TargetVolume = 68000.0
		fmt.Println("✅ 使用默认目标交易量: 68000")
	} else {
	...
	// 验证速度模式
	validSpeedModes := []string{"fast", "normal", "slow"}
	isValidSpeedMode := false
	for _, mode := range validSpeedModes {
		if req.SpeedMode == mode {
			isValidSpeedMode = true
			break
		}
	}
	if !isValidSpeedMode {
		return nil, fmt.Errorf("无效的速度模式: %s，支持的模式: %v", req.SpeedMode, validSpeedModes)
	}
	...
	// 默认启用WebSocket实时价格，禁用缓存
	req.DisableCache = true
	req.UseWebSocket = true
	fmt.Println("✅ 已启用WebSocket实时价格，禁用价格缓存")
...
func sendTradeRequest(node NodeConfig, tradeParams *FlashTradeRequest) (bool, string) {
	...
	if !strings.HasPrefix(nodeIP, "http://") && !strings.HasPrefix(nodeIP, "https://") {
		...
		if !strings.Contains(nodeIP, ":") {
			...
			fmt.Printf("🔌 [%s] 添加默认端口8080: %s\n", node.Name, nodeIP)
		} else {
			nodeIP = "http://" + nodeIP
		}
	} else {
		// 已有协议前缀，检查是否有端口
		if !strings.Contains(strings.Split(nodeIP, "//")[1], ":") {
			// 提取协议部分
			protocol := strings.Split(nodeIP, "//")[0] + "//"
			// 提取主机部分
			host := strings.Split(nodeIP, "//")[1]
			// 添加默认端口
			nodeIP = protocol + host + ":8080"
			fmt.Printf("🔌 [%s] 添加默认端口8080: %s\n", node.Name, nodeIP)
		}
	}
	
	url := fmt.Sprintf("%s/trade", nodeIP)
